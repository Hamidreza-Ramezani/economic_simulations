...
trait SummaryTrait {
  ...
  def updateWasteSummary(categoryName: String, wastedAmount: Int): Unit = {
    ...
    println(Console.RESET + Console.RED + "Wasted Amount " + wasteSummary.formatted(
      s"Vegetable: ${utilities.to2Dec(wasteSummary.Vegetable)} " +
        s"Meat: ${utilities.to2Dec(wasteSummary.Meat)} " +
        s"Snack: ${utilities.to2Dec(wasteSummary.Snack)} " +
        s"Grain: ${utilities.to2Dec(wasteSummary.Grain)} " +
        s"Dairy: ${utilities.to2Dec(wasteSummary.Dairy)}" +
        Console.RESET
    ))
  }
}
