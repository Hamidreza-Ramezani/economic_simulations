...
class FarmerSpec extends FlatSpec with Matchers {
  ...
  "getFreeSpace" should "work" in {
    sectionVegetable1.setShelfCapacity(10)
    supermarket1.shelfCapacity = 10
    assert(farmer.getFreeSpace("Potato") == 9)
  }

  "genNewItem" should "work" in {
    assert(farmer.produce("Item1").getClass.getName == "meta.example.supermarket.goods.Item1")
  }
